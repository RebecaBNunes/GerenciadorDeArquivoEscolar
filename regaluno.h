...
void cadastrar(){
    ...
    do{
        ...
        printf("Dia: ");
        scanf("%hd", &aluno.dataNascimento.tm_mday);
        printf("Mes: ");
        scanf("%hd", &aluno.dataNascimento.tm_mon);
        printf("Ano: ");
        scanf("%hd", &aluno.dataNascimento.tm_year);
...
void consultar(){
    ...
    struct tm data = *localtime(&dataHoje);
    data.tm_year += 1900;
    char ch;
...
void remover(){
    ...
    if(quantidadeDeLinhas < indice && indice < 0){
        printf("Índice maior que quantidade de linhas ou nao valido.\n");
        fclose(arquivo);
        exit(EXIT_FAILURE);
    }
